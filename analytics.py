"""
Analytics layer. Plain SQL over the attempts table -- no Python aggregation.

Each of the four required metrics has an ambiguity in how it is worded. Each is
resolved explicitly below rather than silently, so the numbers can be defended.

1. CONNECTION RATE
   answered contacts / distinct contacts DISPATCHED.
   Denominator is contacts actually dispatched, not cohort size: a contact that
   never got claimed shouldn't count against the rate. In this simulation those
   are the same number, but the distinction matters the moment a campaign is
   cancelled part-way.

2. DISPOSITION BREAKDOWN
   Per COMPLETED ATTEMPT, not per contact. Percentages are of total completed
   attempts. Worth stating, because per-attempt and per-contact breakdowns are
   different numbers and conflating them is a classic reporting bug -- a contact
   that went no_answer, no_answer, answered contributes three rows here, not one.

3. AVERAGE ATTEMPTS BEFORE RESOLUTION
   The assignment says "answered or exhausted retries", which omits contacts that
   ended on a terminal disposition (voicemail). Both readings are reported:
     - headline: all resolved contacts (answered + voicemail + exhausted)
     - strict:   answered + exhausted only, as literally worded
   Computed from contact_outcomes.final_attempt, which is each contact's last
   attempt number.

4. AVERAGE TIME-TO-FIRST-CONNECT (answered contacts only)
   answered attempt's completed_at MINUS that contact's first dispatched_at.
   This is WALL-CLOCK, so it deliberately includes backoff waiting between
   attempts. The business question is "how long from starting on this customer to
   actually reaching them", and time spent waiting out a backoff is a real part of
   that. The alternative -- summing only dial latencies -- measures provider speed,
   not campaign effectiveness. Reported alongside it for contrast.

   (MIN over answered attempts is redundant since `answered` is terminal, so a
   contact has at most one answered row. Used anyway for safety.)
"""
